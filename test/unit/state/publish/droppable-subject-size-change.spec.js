...
[vertical, horizontal].forEach((axis: Axis) => {
  describe(`on the ${axis.direction} axis`, () => {
    ...
    it('should adjust a subject in response to a change', () => {
      const expandedSubjectClient: BoxModel = adjustBox(
        scrollableHome.client,
        patch(axis.line, 10),
      );
      ...
      expect(postUpdateHome.client).toEqual(
        scrollableHomeWithAdjustment.client,
      );
    ...
    it('should only adjust a subject on the main axis (otherwise need to worry about scrollbars)', () => {
      const expandedSubjectClient: BoxModel = adjustBox(
        scrollableHome.client,
        patch(axis.line, 20, 10),
      );
      const expectedSubjectClient: BoxModel = adjustBox(
        scrollableHome.client,
        patch(axis.line, 20, 0),
      );
      ...
      // droppable client subject has changed
      expect(postUpdateHome.client).toEqual(expectedSubjectClient);
      expect(postUpdateHome.client).not.toEqual(
        scrollableHomeWithAdjustment.client,
      );
