...
- `shouldRespectForcePress`: Whether or not the _drag handle_ should respect force press interactions. This defaults to `true` in order to play as nicely as possible with browsers. However, heavy presses can cancel a drag. This can be frustrating for users. By setting `shouldRespectForcePress` to `false`, force press interactions will have `event.preventDefault()` called on them and the user will be able to use any amount of pressure when dragging.
