...
There may be a situation where you want your `Draggable` to contain an interactive element such as a `<button>` or an `<input>`. If you do not do anything the element **will not be interactive**. Interacting with this elements will be used as part of the caluation to start a drag. This is because we call `event.preventDefault` on the `mousedown` event for the `Draggable`. This will prevent the input event from performing its standard actions and interactions. What you will need to do is *opt out* of this behaviour. By doing this the interactive element will not be able to be used to start a drag - but will allow the user to interact with them. There are a few ways you can do this. Here are some suggestions:
